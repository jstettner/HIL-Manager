...
const testCases = [
  ...
  },
  {
    ...
    description: "Does it fry an egg mid-jump or hold steady at subatomic temps?",
    duration: 120,
    priority: "high",
  },
  {
...
async function addTestData() {
  try {
    ...
    let { data: organizations } = await supabase.from('organizations').select('*');
    let organizationId;
    ...
    for (const mapping of testCaseEnvironmentMappings) {
      ...
      if (error) {
        console.error(`Error adding relationship between ${mapping.testcase_id} and ${mapping.environment_id}:`, error);
      }
    }

    // Step 5: Add changesets
    console.log('Adding changesets...');
    ...
    for (const mapping of changesetTestcaseMappings) {
      ...
      if (error) {
        console.error(`Error adding relationship between ${mapping.changeset_id} and ${mapping.testcase_id}:`, error);
      }
    }
